package com.ics.icsoauth2server.helper;

public class ConstantExceptionMessage {

    /**
     *
     * Authorized Exception Message
     *
     * */

    public static final String UNAUTHORIZED = "Un-authorized exception";
    public static final String FORBIDDEN = "Forbidden Exception";
    public static final String PERMISSION_DENIED = "Permission denied!";

    /**
     *
     * Category Exception Message
     *
     * */

    public static final String CATEGORY_UUID_EMPTY = "Category UUID must be initialized";
    public static final String CATEGORY_NAME_EMPTY = "Category name must be provided";





}
